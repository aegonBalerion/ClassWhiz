...
export const createLecture = AsyncHandler(async (req, res) => {
  ...
  const { title, course, files } = req.body;
  const user = req.user;

  if (!title || !course || !files || !Array.isArray(files)) {
    throw new ApiError(400, "title, course, and files[] are required");
  }
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, lecture, "Lecture uploaded successfully"));
});


export const getLecturesForUser = AsyncHandler(async (req, res) => {
  const userId = req.user._id;
  const course = req.query.course; // support course filter
  ...
  if (!lectures || lectures.length === 0) {
    return res.status(200).json(new ApiResponse(200, [], "No lectures found"));
  }
